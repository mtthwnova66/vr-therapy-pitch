// Define initLevel1 so it is only called when the user selects Level 1.
window.initLevel1 = function() {
  console.log('Initializing Level 1: photorealistic scene + VR portal...');
  
  // --- Container and Library Checks ---
  ...
  // --- Loading Manager (for all assets) ---
  ...
  const vrGroup = new THREE.Group();  // Will hold the VR headset
  scene.add(vrGroup);
  const envGroup = new THREE.Group();   // Will hold the table, jar, spider, etc.
  envGroup.visible = false;             // Hidden initially
  scene.add(envGroup);
  
  // --- Create a TextureLoader using the same loadingManager ---
  const textureLoader = new THREE.TextureLoader(loadingManager);
  
  // --- Load Environment Map (optional; if failing, scene.background remains plain) ---
  try {
    const urls = [
    ...
    const loadedCubeTexture = cubeTextureLoader.load(urls);
    scene.environment = loadedCubeTexture;
    // (We still use a plain background for clarity.)
  } catch (e) {
  ...
  const transitionDuration = 5;  // duration (in seconds) for the camera move
  ...
  gltfLoader.load(
    ...
    function(gltf) {
      ...
      // Adjust rotation so its front is toward the viewer.
      // (If it still seems off, try tweaking rotation.x or rotation.z)
      vrHeadset.rotation.y = Math.PI; // 180 degrees
      ...
      // (Adjust the local offset as needed for your model.)
  ...
  // --- Build the Environment (table, jar, spider) exactly as in your original code ---
  ...
  // ------------------------------------------------------------------------
  // We want the camera to start at its initial position and, after a 3-second delay,
  // smoothly move to the VR headset’s left eye position (targetCamPos), then fade out the headset.
  ...
  function finalizeScene() {
    function animate() {
      ...
      // Slowly rotate the VR headset for proper presentation.
      if (vrHeadset) {
      ...
      if (transitionStarted && vrHeadset && targetCamPos) {
        ...
        // Focus camera on the jar (centered at (0, 0.75, 0))
        camera.lookAt(new THREE.Vector3(0, 0.75, 0));
        // Once transition completes, fade out the VR headset and reveal the environment.
        if (t === 1 && vrHeadset) {
    ...
    function addUIControls() {
      ...
      console.log('UI controls added. Scene setup completed.');
    }
  ...
  setTimeout(() => {
    if (texturesLoaded < requiredTextures) {
      console.warn('Not all textures loaded in time, using fallbacks.');
