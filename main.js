// Define the Level 1 initialization function.
// Call this function only when the user presses the "Level 1" button.
window.initLevel1 = function() {
  ...
  try {
    // ------------------------------------------------------------------------
    // LOADING MESSAGE & LOADING MANAGER
    // ------------------------------------------------------------------------
    ...
    // (Optional) Create a simple loading bar overlay
    ...
    // ------------------------------------------------------------------------
    // LIGHTING SETUP (Same as original)
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // The VR group (vrGroup) holds the large VR headset.
    // The environment group (envGroup) holds the table, jar, spider, etc.
    const vrGroup = new THREE.Group();
    scene.add(vrGroup);
    
    const envGroup = new THREE.Group();
    envGroup.visible = false;  // Hide until transition completes
    scene.add(envGroup);
    ...
    const transitionDuration = 5;  // Zoom transition lasts 5 seconds
    
    gltfLoader.load(
      'oculus_quest_vr_headset.glb',
      function(gltf) {
        vrHeadset = gltf.scene;
        // Scale up so it's large
        vrHeadset.scale.set(2, 2, 2);
        // Rotate so its front faces the user.
        // Adjust the rotation if needed (try tweaking rotation.x and rotation.y)
        ...
        // Compute target camera position from headset's left eye.
        // Here, we assume the left eye local offset is (-0.15, 0.0, 0.2) relative to the headset.
    ...
    // ------------------------------------------------------------------------
    // BUILD THE PHOTOREALISTIC ENVIRONMENT (TABLE, JAR, SPIDER)
    ...
    function createTable() {
      ...
      table.position.y = -0.1; // Slightly below jar
    ...
    // ------------------------------------------------------------------------
    // CAMERA TRANSITION & VR FADE
    // ------------------------------------------------------------------------
    // The camera starts at its initial position. After a 3-second delay (if the VR headset is loaded),
    // it interpolates over 5 seconds to the position of the headset's left eye.
    const checkVRInterval = setInterval(() => {
      if (vrHeadset) {
        // Assume the left eye in the headset’s local space is at (-0.15, 0.0, 0.2).
    ...
    function finalizeScene() {
      function animate() {
        ...
        // Slowly rotate the VR headset for a proper presentation
        if (vrHeadset) {
        ...
        if (transitionStarted && vrHeadset && targetCamPos) {
          ...
          // Aim the camera at the jar (centered at (0, 0.75, 0))
          camera.lookAt(new THREE.Vector3(0, 0.75, 0));
          // Once the transition is complete, fade out the VR headset and reveal the environment
          if (t === 1 && vrHeadset) {
      ...
      // Add UI Controls for Fullscreen and Auto-Rotate
      ...
      console.log('Scene setup completed (VR + spider).');
    }
    
    // If textures do not load within 5 seconds, use fallback
    setTimeout(() => {
      if (texturesLoaded < requiredTextures) {
